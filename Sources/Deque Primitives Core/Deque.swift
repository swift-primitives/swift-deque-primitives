...
public struct Deque<Element: ~Copyable>: ~Copyable {
    ...
    @usableFromInline
    package typealias _Header = (count: Int, head: Int, bufferCapacity: Int)
    ...
    @usableFromInline
    package final class Storage: ManagedBuffer<_Header, Element> {
...
// MARK: - Bounded Subscript (Copyable elements)
// NOTE: Subscript must be in this file (not Deque.Index.swift) to avoid ~Copyable
// constraint poisoning during emit-module when extensions are in separate files.
